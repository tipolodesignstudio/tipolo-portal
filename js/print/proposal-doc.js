// The proposal document, as markup.
//
...
// lookalike — it's the same HTML under the same stylesheet (css/document.css), just
// inside a paper-sized box instead of on a sheet of paper.
//
// Tokens are resolved here, so the preview shows the client's real name rather than
// {{client.name}}.
  ...
  return `
    <div class="doc">
      <div class="doc-head">
        <div class="biz">
          ${settings.logo_url
            ? `<img src="${escapeHtml(settings.logo_url)}" alt="" style="max-height:54px;margin-bottom:6px" />`
      ...
      <div class="parties">
        ...
        <div>
          ...
          <div>${escapeHtml(p.project_scope || "")}</div>
        </div>
      </div>
