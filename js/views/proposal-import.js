// Import a finished proposal from a PDF.
//
// The PDF is the proposal. It is not taken apart and rebuilt in the builder — it is
// attached and shown as the document. What the portal takes out of it is the two things
// it needs to work with: who it is for, and what it is worth. The fee schedule becomes
// the proposal's line items, which is what later sets the project's budget and the
// Budget column of its invoices.
//
// The prose is still read, but only so it can be harvested into a reusable template if
// you want one — it never becomes the proposal's own text.
//
// Nothing is written to the database until "Create proposal" is clicked — the reader
// only ever fills in a form for you to check.
...
export async function render(root, ctx) {
  ...
  root.innerHTML = `
    <div class="page-head">
      <div>
        ...
        <div class="muted">The PDF becomes the proposal. Only the client and the fees come into the portal — check them over before creating it.</div>
      </div>
    </div>
    <div id="stage"></div>`;
    ...
    stage.innerHTML = `
      ...
      <div class="card">
        <div id="drop" class="empty" style="border:2px dashed var(--border);border-radius:10px;
          ...
          <h3>Drop a finished proposal PDF here</h3>
    ...
    stage.innerHTML = `
      ...
      <div class="card" style="background:var(--bg-alt)">
        <div class="cluster" style="gap:10px">
          <strong style="font-size:.92rem">The PDF is the proposal.</strong>
          <span class="faint" style="font-size:.88rem">It is attached as the document and
            shown as it is — the wording is not rebuilt here. Only the client and the
            fees above come into the portal.</span>
        </div>
      </div>

      <div class="cluster right" style="justify-content:flex-end;margin-top:8px">
        <button class="btn ghost" data-template>Save the wording as a template</button>
        <button class="btn" data-create>Create proposal</button>
      </div>`;
  ...
  on(root, "click", "[data-create]", async (e, btn) => {
    ...
    try {
      ...
      // The PDF is the document, so it is not optional — without it the proposal would
      // be a fee schedule with nothing to read behind it.
      let sourcePath = null;
      try { sourcePath = await uploadProposalSource(file); }
      catch (err) {
        btn.disabled = false;
        btn.textContent = label;
        return toastErr(`Couldn't attach the PDF, so nothing was created: ${err.message}`);
      }
